...
> Ethan Z. Welty, Christian E. Torgersen, Samuel J. Brenkman, Jeffrey D. Duda, and Jonathan B. Armstrong. 2015. Multiscale analysis of river networks using the linbin R package. North American Journal of Fisheries Management.
...
bins.b <- seq_bin_events(event_range(e), bin.count = 33, adaptive = FALSE)
bins.c <- seq_bin_events(event_coverage(e), bin.count = 20, adaptive = FALSE)
bins.d <- seq_bin_events(event_coverage(e.filled), bin.count = 20, adaptive = TRUE)
...
mtext('Distance upstream (km)', side = 1, outer = TRUE, cex = 0.9)
mtext('Wetted width (m)', side = 2, outer = TRUE, cex = 0.9)
```
...
bins <- seq_bin_events(event_range(e), bin.length = bin.lengths / 1000) # km
...
mtext('Distance upstream (km)', side = 1, line = 1, outer = TRUE)
mtext('Trout abundance', side = 2, line = 1, outer = TRUE)
```
...
# Design bins
bins = seq_bin_events(event_range(e.net), bin.count = 10)
...
# Plot binned data
plot_events(e.bins, group.col = 'group', data.cols = fields, 
            oma = c(3, 3, 2, 2), mar = c(2, 4, 1.5, 0.5))
mtext(c('Distance upstream (km)\nmainstem', 'Distance upstream (km)\nnetwork'), 
      side = 1, line = 2, outer = TRUE, at = c(0.25, 0.75))
mtext(c('Depth (m)', 'Proportion', 'IP', 'IP', 'IP'), 
      side = 2, line = 1, outer = TRUE, at = seq(0, 0.8, 0.2) + 0.1)
```
...
bins = seq_bin_events(event_range(e.motion), bin.length = 3600)
...
plot_events(e.motion.bins, data.cols = "fish.1.norm", yticks = c(0, 1, 2), 
            col = par("fg"), ylim = c(0, 2), plot.grid = TRUE, xpd = FALSE, main = '',
            xticks = week.ticks, xlabels = week.labels, cex.axis = 0.9, oma = c(3, 2, 1, 2))
mtext('Date (2008)', side = 1, line = 1, outer = TRUE)
mtext('Relative abundance', side = 2, outer = TRUE)
