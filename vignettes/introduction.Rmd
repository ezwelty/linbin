...
plot_events(e.bins, xticks = axTicks, col = "grey", border = par("bg"))
```
...
plot_events(metrics, group.col = "g", data.cols = "n", dim = c(4, 1), xlim = c(0, 100), col = 'grey', main = c("Range", "Coverage", "Overlaps", "Gaps"), lwd = 0.5, mar = rep(1.5, 4), oma = rep(1, 4))
```
...
plot_events(e.bins, data.cols = "mean.width", col = "grey", border = "#666666", 
            ylim = c(0, 56), main = "", oma = rep(1, 4), mar = rep(0, 4), 
            xticks = NA, yticks = NA, cex.axis = 0.75, xpd = NA)
```
...
```{r}
bins = seq_bin_events(event_range(e), bin.count = 33)
```

```{r, echo = FALSE, fig.width = 4.5, fig.height = 1.25}
e.bins = sample_events(e, bins, list(weighted.mean, "mean.width", "unit.length"), scaled.cols = "unit.length")
plot_events(e.bins, data.cols = "mean.width", col = "grey", border = "#666666", ylim = c(0, 56), xpd = NA, main = "", oma = c(1, 1, 1,1), mar = rep(0, 4), cex.axis = 0.75, xticks = NA, yticks = NA)
```
...
```{r}
bins = seq_bin_events(event_coverage(e), bin.count = 20)
```
...
```{r}
e.filled = fill_event_gaps(e, max.length = 1) # fill any small gaps first
bins = seq_bin_events(event_coverage(e.filled), bin.count = 20, adaptive = TRUE)
```
...
```{r}
e = simple
bins = seq_bin_events(event_range(e), bin.count = 1)
```
...
e = simple
bins = seq_bin_events(event_range(e), bin.count = c(1, 4, 16)) # appends a "group" column
e.bins = sample_events(e, bins, list(mean, c('x', 'y'), na.rm = TRUE), list(sum, 'x', na.rm = TRUE))
